...
DATABASE_URL = "sqlite:///cs_eval_r1.db"  # TODO: rename it based on annotation round e.g. pilot_db
...
init_database('data/eval_sample_r1.tsv', engine)
        ...
        # if subset is available, show annotation task
            ...
            if 0 <= current_index < subset_size:
                ...
                if pair_id not in user_info1.pairs.split(", "):
                    existing_round1 = st.session_state.subset[current_index][1]
                else:
                    existing_round1 = ""
                ...
                cs_r1 = [
                    ("cs_prompting_0", pair.cs_prompting_0),
                    ("cs_strategy_single", pair.cs_strategy_single),
                    ("cs_strategy_multi", pair.cs_strategy_multi),
                    ("cs_strategy_form", pair.cs_strategy_form),
                    ("cs_strategy_form_target",pair.cs_strategy_form_target),
                ]
                
                if not st.session_state.shuffled:
                    random.seed(st.session_state.page)
                    st.session_state.shuffled_list = random.sample(cs_r1, k=len(cs_r1))
                    st.session_state.shuffled = True  # Set the flag to indicate shuffling has been done
                
                cs_r1_shuffled = st.session_state.shuffled_list
                ...
                with st.container(border=True):
                    ...
                    with col2_0_0:
                        cs_r1_0_0 = st.feedback(options="thumbs", key=f"cs_r1_0_0_{user_id}_{pair_id}_{current_index}")
                    
                    col1_0_1, col2_0_1= st.columns([8,1])
                    with col1_0_1:
                        st.markdown("**Does the response feel persuasive or effective?**")
                    with col2_0_1:
                        cs_r1_0_1 = st.feedback(options="thumbs", key=f"cs_r1_0_1_{user_id}_{pair_id}_{current_index}")

                    col1_0_2, col2_0_2= st.columns([8,1])
                    with col1_0_2:
                        st.markdown("**Do you think the response could promote positive and educational dialogue?**")
                    with col2_0_2:
                        cs_r1_0_2 = st.feedback(options="thumbs", key=f"cs_r1_0_2_{user_id}_{pair_id}_{current_index}")
                ...
                with st.container(border=True):
                    ...
                    with col1_1_0:
                        st.markdown("**Does the response directly and appropriately address the harmful content?**")
                    with col2_1_0:
                        cs_r1_1_0 = st.feedback(options="thumbs", key=f"cs_r1_1_0_{user_id}_{pair_id}_{current_index}")
                    
                    col1_1_1, col2_1_1= st.columns([8,1])
                    with col1_1_1:
                        st.markdown("**Does the response feel persuasive or effective?**")
                    with col2_1_1:
                        cs_r1_1_1 = st.feedback(options="thumbs", key=f"cs_r1_1_1_{user_id}_{pair_id}_{current_index}")
                    
                    col1_1_2, col2_1_2= st.columns([8,1])
                    with col1_1_2:
                        st.markdown("**Do you think the response could promote positive and educational dialogue?**")
                    with col2_1_2:
                        cs_r1_1_2 = st.feedback(options="thumbs", key=f"cs_r1_1_2_{user_id}_{pair_id}_{current_index}")
                ...
                with st.container(border=True):
                    ...
                    with col1_2_0:
                        st.markdown("**Does the response directly and appropriately address the harmful content?**")
                    with col2_2_0:
                        cs_r1_2_0 = st.feedback(options="thumbs", key=f"cs_r1_2_0_{user_id}_{pair_id}_{current_index}")
                    
                    col1_2_1, col2_2_1= st.columns([8,1])
                    with col1_2_1:
                        st.markdown("**Does the response feel persuasive or effective?**")
                    with col2_2_1:
                        cs_r1_2_1 = st.feedback(options="thumbs", key=f"cs_r1_2_1_{user_id}_{pair_id}_{current_index}")
                    
                    col1_2_2, col2_2_2= st.columns([8,1])
                    with col1_2_2:
                        st.markdown("**Do you think the response could promote positive and educational dialogue?**")
                    with col2_2_2:
                        cs_r1_2_2 = st.feedback(options="thumbs", key=f"cs_r1_2_2_{user_id}_{pair_id}_{current_index}")
                ...
                with st.container(border=True):
                    ...
                    with col1_3_0:
                        st.markdown("**Does the response directly and appropriately address the harmful content?**")
                    with col2_3_0:
                        cs_r1_3_0 = st.feedback(options="thumbs", key=f"cs_r1_3_0_{user_id}_{pair_id}_{current_index}")
                    
                    col1_3_1, col2_3_1= st.columns([8,1])
                    with col1_3_1:
                        st.markdown("**Does the response feel persuasive or effective?**")
                    with col2_3_1:
                        cs_r1_3_1 = st.feedback(options="thumbs", key=f"cs_r1_3_1_{user_id}_{pair_id}_{current_index}")

                    col1_3_2, col2_3_2= st.columns([8,1])
                    with col1_3_2:
                        st.markdown("**Do you think the response could promote positive and educational dialogue?**")
                    with col2_3_2:
                        cs_r1_3_2 = st.feedback(options="thumbs", key=f"cs_r1_3_2_{user_id}_{pair_id}_{current_index}")
                        
                with st.container(border=True):
                    st.markdown(f"**Counterspeech 5:**  \n  {cs_r1_shuffled[4][1]}")
                    st.markdown("<br>", unsafe_allow_html=True)
                    col1_4_0, col2_4_0= st.columns([8,1])
                    with col1_4_0:
                        st.markdown("**Does the response directly and appropriately address the harmful content?**")
                    with col2_4_0:
                        cs_r1_4_0 = st.feedback(options="thumbs", key=f"cs_r1_4_0_{user_id}_{pair_id}_{current_index}")
                    
                    col1_4_1, col2_4_1= st.columns([8,1])
                    with col1_4_1:
                        st.markdown("**Does the response feel persuasive or effective?**")
                    with col2_4_1:
                        cs_r1_4_1 = st.feedback(options="thumbs", key=f"cs_r1_4_1_{user_id}_{pair_id}_{current_index}")
                    
                    col1_4_2, col2_4_2= st.columns([8,1])
                    with col1_4_2:
                        st.markdown("**Do you think the response could promote positive and educational dialogue?**")
                    with col2_4_2:
                        cs_r1_4_2 = st.feedback(options="thumbs", key=f"cs_r1_4_2_{user_id}_{pair_id}_{current_index}")
                
                cs_r1_list = [
                    (str(cs_r1_shuffled[0][0]), str(cs_r1_0_0), str(cs_r1_0_1), str(cs_r1_0_2)), 
                    (str(cs_r1_shuffled[1][0]), str(cs_r1_1_0), str(cs_r1_1_1), str(cs_r1_1_2)), 
                    (str(cs_r1_shuffled[2][0]), str(cs_r1_2_0), str(cs_r1_2_1), str(cs_r1_2_2)), 
                    (str(cs_r1_shuffled[3][0]), str(cs_r1_3_0), str(cs_r1_3_1), str(cs_r1_3_2)), 
                    (str(cs_r1_shuffled[4][0]), str(cs_r1_4_0), str(cs_r1_4_1), str(cs_r1_4_2)), 
                ]
                    
                is_correctly_selected = check_selection(cs_r1_list)
                
                selected_cs_r1_list = cs_r1_list.copy()
                selected_cs_r1_list = [str(item) for item in selected_cs_r1_list] 
                selected_cs_r1_text = "@@@".join(selected_cs_r1_list)  # Convert to string for storage

                # Update cs_r1 in session_state subset -> convert tuple to list, update it, then assign back
                temp_cs_r1_list = list(st.session_state.subset[current_index])  
                temp_cs_r1_list[1] = selected_cs_r1_text  
                st.session_state.subset[current_index] = tuple(temp_cs_r1_list)  
                st.markdown("<br>", unsafe_allow_html=True)
