...
NUM_ANNOTATOR_PER_ITEM = 5  # default=3 for maximum annotator per item, add 1 for tolerance 

CUSTOMISED_USER_ID = "423aaabc"

# Prolific completion code - TODO: check code every time before deployment
COMPLETION_CODE = "CQVNS9MA"
COMPLETION_URL = "https://app.prolific.com/submissions/complete?cc=CQVNS9MA"
SCREEN_OUT_CODE = "CRPOBWEN"  
SCREEN_OUT_URL = "https://app.prolific.com/submissions/complete?cc=CRPOBWEN"
NO_DATA_CODE = "C1MPYTWA"
NO_DATA_URL = "https://app.prolific.com/submissions/complete?cc=C1MPYTWA"

# Create the SQLite engine and session
DATABASE_URL = "sqlite:///cs_eval_r3.db"  # TODO: rename it based on annotation round e.g. pilot_db
...
init_database('data/eval_sample_r3.tsv', engine)
...
def check_selection(selected_list):
    """
    Check if answer is given for all counterspeech texts.
    """
    st.markdown("<br>", unsafe_allow_html=True)
...
def main():
    ...
    if st.session_state.page >= 0:
        ...
        with st.sidebar:
            ...
            st.subheader("🟢 Response Explanations")
            st.markdown("""
        ...
        # if subset is available, show annotation task
            ...
            if 0 <= current_index < subset_size:
                ...
                if pair_id not in user_info1.pairs.split(", "):
                    existing_round3 = st.session_state.subset[current_index][1]
                else:
                    existing_round3 = ""
                ...
                cs_r3 = [
                    ...
                    ("response_3", pair.response_3),
                    ("response_4", pair.response_4),
                ]

                if not st.session_state.shuffled:
                    random.seed(st.session_state.page)  
                    st.session_state.shuffled_list = random.sample(cs_r3, k=len(cs_r3))
                    st.session_state.shuffled = True  # Set the flag to indicate shuffling has been done
                cs_r3_shuffled = st.session_state.shuffled_list
                
                with st.container(border=True):
                    st.markdown(f"{cs_r3_shuffled[0][1]}")
                    cs_r3_0_0 = st.feedback(options="thumbs", key=f"cs_r3_0_0_{user_id}_{pair_id}_{current_index}")
                with st.container(border=True):
                    st.markdown(f"{cs_r3_shuffled[1][1]}")
                    cs_r3_1_0 = st.feedback(options="thumbs", key=f"cs_r3_1_0_{user_id}_{pair_id}_{current_index}")
                with st.container(border=True):
                    st.markdown(f"{cs_r3_shuffled[2][1]}")
                    cs_r3_2_0 = st.feedback(options="thumbs", key=f"cs_r3_2_0_{user_id}_{pair_id}_{current_index}")
                with st.container(border=True):
                    st.markdown(f"{cs_r3_shuffled[3][1]}")
                    cs_r3_3_0 = st.feedback(options="thumbs", key=f"cs_r3_3_0_{user_id}_{pair_id}_{current_index}")
  
                cs_r3_list = [
                    (str(cs_r3_shuffled[0][0]), str(cs_r3_0_0)),
                    (str(cs_r3_shuffled[1][0]), str(cs_r3_1_0)),
                    (str(cs_r3_shuffled[2][0]), str(cs_r3_2_0)),
                    (str(cs_r3_shuffled[3][0]), str(cs_r3_3_0)),
                ]
                    
                is_correctly_selected = check_selection(cs_r3_list)
                
                selected_cs_r3_list = cs_r3_list.copy()
                selected_cs_r3_list = [str(item) for item in selected_cs_r3_list] 
                selected_cs_r3_text = "@@@".join(selected_cs_r3_list)  # Convert to string for storage

                # Update cs_r3 in session_state subset -> convert tuple to list, update it, then assign back
                temp_cs_r3_list = list(st.session_state.subset[current_index])  
                temp_cs_r3_list[1] = selected_cs_r3_text  
                st.session_state.subset[current_index] = tuple(temp_cs_r3_list)  
                st.markdown("<br>", unsafe_allow_html=True)
